...
from modules.dialog_window.main_config.main_config_dialog import MainConfigDialog
from modules.dialog_window.main_hvip.main_hvip_dialog import MainHvipDialog
from modules.dialog_window.main_mpp.main_mpp_dialog import MainMppControlDialog
from modules.dialog_window.main_mpp.main_trapezoid.main_trapezoid_dialog import MainTrapezoidDialog
...
class Engine(QtWidgets.QMainWindow, QThread):
    ...
    def __init__(self) -> None:
        ...
        loadUi(Path(__file__).parent.joinpath("/fronend/MainWindow5.ui"), self)
        # loadUi(os.path.join(os.path.dirname(__file__),  '/fronend/MainWindow5.ui'), self)
    ...
    def thread_readWaveform_adc_flow(self) -> None:
        ...
        while 1:
            if self.flag_readWaveform_trig != 1:
                ...
                self.client.write_registers(
                    address=0x0000, values=[0x0009, 0x0000], slave=self.mpp_id
                )  # запуск регистрации
                self.client.read_holding_registers(address=0x0000, count=1, slave=self.mpp_id)  # начать измерение
                log_s(self.send_handler.mess)
            waveformA = self.readWaveform_adcA()
