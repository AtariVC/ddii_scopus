...
## 13. Оформление докстринг

Писать только кратко и саму суть без лишнего описания. Название функции должно точно отрадъжать то что она делает
Пимер:
``` py
    async def _request_ch_hvip(self, ch: int) -> bytes:
        """Прочитать регистры HVIP одного канала.
        Args:
            ch(int): номер канала HVIP [0..HVIP_CH_NUM-1].
        Returns:
            Сырые байты окна регистров (по 2 байта на регистр) или ``b"-1"`` при
            ошибке / отсутствии командного интерфейса.
        """
    ...
```