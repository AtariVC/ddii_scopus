...
# from MainUIRenderer_trapezoid_dialog import MainUIRendererTrapezoidFilter
import os
...
class MainMppControlDialog(QtWidgets.QDialog):
    ...
    def pushButton_control_pips_handler(self) -> None:
        ...
        self.root.send_comand_Modbus(dev_id=self.CM_ID, f_code=16, reg_cnt=self.CNTRL_PIPS, comand=level_pips)
    ...
    def pushButton_control_sipm_handler(self) -> None:
        ...
        self.root.send_comand_Modbus(dev_id=self.CM_ID, f_code=self.F_CODE, reg_cnt=self.CNTRL_SIPM, comand=level_sipm)
    ...
    def pushButton_ok_handler(self) -> None:
        self.root.send_comand_Modbus(dev_id=self.CM_ID, f_code=self.F_CODE, reg_cnt=self.MMP_ENABLE, comand=0)
        self.root.get_transaction_Modbus(2)

    def pushButton_request_level_mpp_pips_handler(self) -> None:
        self.root.send_comand_Modbus(dev_id=self.CM_ID, f_code=self.F_CODE, reg_cnt=self.REQUEST_PIPS, comand=0)
    ...
    def pushButton_request_level_mpp_sipm_handler(self) -> None:
        self.root.send_comand_Modbus(dev_id=self.CM_ID, f_code=self.F_CODE, reg_cnt=self.REQUEST_SIPM, comand=0)
        transaction = self.root.get_transaction_Modbus(2)
